...
function checkNumbersAgainstHistory(){
  ...
  try{
    checkMainNumbers(input);
  } catch (Exception) {
    console.log('Wrong input or no input given\n'+Exception.message);
    deferred.resolve();
    return deferred.promise;
  }
  ...
  connection.query(
    ...
    function(err, result) {
      ...
      //output result
      if (result.length !== 0) {
        console.log('\nThe numbers',input, 'have appeared in the draw of ');
        for (var i = 0, row; i < result.length; i++) {
          row = result[i];
          console.log(row.date.toDateString(),'\n');
        }
      } else {
        console.log('\nThe numbers',input,'have never been drawn yet!\n');
      }
...
function checkMainNumbers(mains) {
  var check;
...
/**
 * transforms an array of string integers into integer array.
 *
 * @param array array of strings integers
 * @return {Array} array of parsed integers
 */
